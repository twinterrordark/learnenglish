...
const ALLOWED_ORIGINS = new Set(String(process.env.ALLOWED_ORIGINS ||
  "https://twinterrordark.github.io,http://localhost:8765,http://127.0.0.1:8765")
...
/* Yönetici anahtarı. Sunucu açılışında ekrana yazılır; yalnızca konsolu gören
   kişi, yani sunucuyu çalıştıran sen bilirsin. Anahtar bir kereliğine üretilip
   .admin-key dosyasına kaydedilir; böylece sunucu her açılıp kapandığında
   yeniden şifre girmek zorunda kalmazsın.

   NOT: "istek 127.0.0.1'den geliyorsa yöneticidir" YAPILAMAZ. Tünel açıkken
   cloudflared bütün istekleri sunucuya localhost'tan iletir, yani o kontrol
   internetten giren herkesi yönetici yapardı. */
const KEYFILE     = path.join(ROOT, ".admin-key");
function loadOrCreateAdminKey() {
  if (process.env.ADMIN_KEY) return String(process.env.ADMIN_KEY).trim();
  try {
  ...
  try { fs.writeFileSync(KEYFILE, k + "\n", "utf8"); } catch (e) {}
  return k;
}
...
const IDLE_MS = 10000;   /* bu süre yoklamayan kişi ayrılmış sayılır */
try {
...
/* Anahtar karşılaştırması sabit sürede yapılır; ayrıca arka arkaya yanlış
   denemelerde kısa süre kilitlenir, böylece deneme yanılma işlemez. */
let adminFails = 0, adminBlockUntil = 0;
function isAdmin(k) {
  if (Date.now() < adminBlockUntil) return false;
  const a = Buffer.from(String(k || ""), "utf8");
  const b = Buffer.from(ADMIN_KEY, "utf8");
  const ok = a.length === b.length && crypto.timingSafeEqual(a, b);
  if (ok) { adminFails = 0; return true; }
  if (++adminFails >= 8) { adminBlockUntil = Date.now() + 60000; adminFails = 0; }
  return false;
}
...
const server = http.createServer(async (req, res) => {
  ...
  /* GitHub Pages HTTPS frontend -> Render API CORS. */
  ...
  /* Sekme kapanırken tarayıcı buraya bir istek bırakır (sendBeacon).
     Böylece kişi listeden beklemeden hemen düşer. */
  if (p === "/api/chat/leave" && req.method === "POST") {
    let body = {};
  ...
  /* ---------- yönetici ---------- */
  if (p.startsWith("/api/chat/admin/") && req.method === "POST") {
    let body = {};
    try { body = await readBody(req); } catch (e) { return json(res, 400, { error: "Geçersiz istek" }); }
    if (!isAdmin(body.key)) return json(res, 403, { error: "Yönetici anahtarı hatalı" });

    const islem = p.slice("/api/chat/admin/".length);

    if (islem === "check") return json(res, 200, { ok: true });

    if (islem === "delete") {
      const hedef = String(body.msgId || "");
      const vardi = messages.some(m => m.id === hedef) || events.some(e => e.id === hedef);
      messages = messages.filter(m => m.id !== hedef);
      events   = events.filter(e => e.id !== hedef);
      persist();
      pushEvent({ id: crypto.randomUUID(), kind: "del", ch: "*", target: hedef, ts: Date.now() });
      return json(res, 200, { ok: true, silindi: vardi });
    }

    if (islem === "clear") {
      const kac = messages.length;
      messages = []; events = [];
      persist();
      pushEvent({ id: crypto.randomUUID(), kind: "clear", ch: "*", ts: Date.now() });
      return json(res, 200, { ok: true, silinen: kac });
    }

    return json(res, 404, { error: "Bilinmeyen yönetici işlemi" });
  }
...
server.listen(PORT, "0.0.0.0", () => {
  ...
  console.log("  Yönetici anahtarı: " + ADMIN_KEY + "   (mesaj silmek için; kimseye verme)");
